...
Elm.OnInput = Elm.OnInput || {};
Elm.OnInput.make = function (_elm) {
   "use strict";
   _elm.OnInput = _elm.OnInput || {};
   if (_elm.OnInput.values) return _elm.OnInput.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   return _elm.OnInput.values = {_op: _op,onInput: onInput};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $List = Elm.List.make(_elm),
   $Maybe = Elm.Maybe.make(_elm),
   $OnInput = Elm.OnInput.make(_elm),
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $StartApp$Simple = Elm.StartApp.Simple.make(_elm);
   var _op = {};
   var filterVisibleTasks = F2(function (address,model) {
      var _p0 = model.filter;
      switch (_p0.ctor)
      {case "All": return model.tasks;
         case "Completed": return A2($List.filter,function (t) {    return _U.eq(t.completed,true);},model.tasks);
         default: return A2($List.filter,function (t) {    return _U.eq(t.completed,false);},model.tasks);}
   });
   var crossedOut = function (completed) {
      var _p1 = completed;
      if (_p1 === true) {
            return $Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "text-decoration",_1: "line-through"}]));
         } else {
            return $Html$Attributes.style(_U.list([]));
         }
   };
   var UpdateTaskInput = function (a) {    return {ctor: "UpdateTaskInput",_0: a};};
   var AddTask = {ctor: "AddTask"};
   var taskForm = function (address) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.input,_U.list([$Html$Attributes.type$("text"),A2($OnInput.onInput,address,UpdateTaskInput)]),_U.list([]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,AddTask)]),_U.list([$Html.text("Add Task")]))]));
   };
   var Filter = function (a) {    return {ctor: "Filter",_0: a};};
   var DeleteTask = function (a) {    return {ctor: "DeleteTask",_0: a};};
   var ToggleCompleted = function (a) {    return {ctor: "ToggleCompleted",_0: a};};
   var taskLi = F2(function (address,task) {
      return A2($Html.li,_U.list([crossedOut(task.completed),A2($Html$Events.onClick,address,ToggleCompleted(task.id))]),_U.list([$Html.text(task.name)]));
   });
   var taskList = F2(function (address,model) {    return A2($Html.ul,_U.list([]),A2($List.map,taskLi(address),A2(filterVisibleTasks,address,model)));});
   var NoOp = {ctor: "NoOp"};
   var Pending = {ctor: "Pending"};
   var Completed = {ctor: "Completed"};
   var All = {ctor: "All"};
   var filterButtons = function (address) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A2($Html.button,_U.list([A2($Html$Events.onClick,address,Filter(All))]),_U.list([$Html.text("All")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Filter(Pending))]),_U.list([$Html.text("Pending")]))
              ,A2($Html.button,_U.list([A2($Html$Events.onClick,address,Filter(Completed))]),_U.list([$Html.text("Completed")]))]));
   };
   var view = F2(function (address,model) {
      return A2($Html.div,_U.list([]),_U.list([A2(taskList,address,model),taskForm(address),filterButtons(address)]));
   });
   var Task = F3(function (a,b,c) {    return {name: a,completed: b,id: c};});
   var initialModel = {tasks: _U.list([A3(Task,"get milk",true,1),A3(Task,"call Mom",false,2),A3(Task,"learn Elm",false,3)])
                      ,filter: All
                      ,nextId: 4
                      ,taskInput: ""};
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return model;
         case "ToggleCompleted": var updateTask = function (t) {    return _U.eq(t.id,_p2._0) ? _U.update(t,{completed: $Basics.not(t.completed)}) : t;};
           return _U.update(model,{tasks: A2($List.map,updateTask,model.tasks)});
         case "DeleteTask": return _U.update(model,{tasks: A2($List.filter,function (t) {    return !_U.eq(t.id,_p2._0);},model.tasks)});
         case "Filter": return _U.update(model,{filter: _p2._0});
         case "AddTask": var taskToAdd = A3(Task,model.taskInput,false,model.nextId);
           return _U.update(model,{tasks: A2($List._op["::"],taskToAdd,model.tasks),nextId: model.nextId + 1,taskInput: ""});
         default: return _U.update(model,{taskInput: _p2._0});}
   });
   ...
   return _elm.Main.values = {_op: _op
                             ...
                             ,taskLi: taskLi
                             ,filterVisibleTasks: filterVisibleTasks
                             ,taskList: taskList
                             ,taskForm: taskForm
